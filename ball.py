...
canvas = Canvas(root, bg="blue",width=600, height=600)
canvas.pack()
...
def score_board():
    ...
    canvas2 = Canvas(root2,bg="blue",width=400,height=400)
    canvas2.pack()

    w = Label(canvas2,text="\nOOPS...GAME IS OVER\n\nYOUR SCORE IS " + str(score) + " unit\n\n")
    w.pack()
...
def main():
    ...
    bar1=bar(canvas,0,560,40,575)

    # defining the text,colour of buttons and
    # also define the action after click on
    # the button by calling suitable methods
    button1 = Button(canvas,text="right>>", bg="green",command=lambda:bar1.move_bar(1))

    # placing the buttons at suitable location on the canvas
    button1.place(x=300,y=580)

    button2 = Button(canvas,text="<<left",bg="green", command=lambda:bar1.move_bar(0))
    button2.place(x=230,y=580)
